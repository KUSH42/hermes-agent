...
- **ASSIST axis scope (CC-12).** The `assist: reactive[AssistKind]` field on `HermesInput` is the single source of truth for the persistent ASSIST state (`NONE`, `GHOST`, `OVERLAY`, `PICKER`). Transient channels (hint flashes, error flashes) are NOT ASSIST states — they are feedback-service concerns. `_resolve_assist()` is the single write site; no other method may assign `self.assist` directly. This boundary was established by SPEC-ASS (2026-05-02).
...
1. **COMPLETION mode does not change the placeholder.** ~~Every other mode rewrites the placeholder; COMPLETION leaves the underlying mode's placeholder visible while the overlay floats above. Structural cause: `_refresh_placeholder()` does not consult `_completion_overlay_visible()`.~~ **CLOSED 2026-05-02** — superseded by `assist: reactive[AssistKind]` (SPEC-ASS). `_refresh_placeholder()` now reads `self.assist == AssistKind.OVERLAY` and leaves the placeholder unchanged for OVERLAY (by design — the overlay visual is sufficient); PICKER gets a `(skill)` placeholder.

   **1b.** ~~Related: opening the skill picker (`PICKER` ASSIST) on a `$`-fragment does not explicitly dismiss an inline overlay if one is somehow still mounted. Coexistence is structurally improbable but not guarded.~~ **CLOSED 2026-05-02** — `_resolve_assist()` is now the single write site and tears down the previous ASSIST state (calling `_hide_completion_overlay()` when current is OVERLAY) before activating PICKER.  *Owner: composer.*
...
4. ~~**ASSIST has no central resolver.** `GHOST`, `OVERLAY`, `PICKER`, `HINT_FLASH`, `ERROR_FLASH` are mutually exclusive (mostly) but the exclusion is enforced at each write site. A central `_resolve_assist()` would replace several inline guards and make 1b structurally impossible.~~ **CLOSED 2026-05-02** — `_resolve_assist()` refactored to single-write-site pattern; `self.assist = target` is the only assignment site, protected by `AssistWriteVisitor` AST gate in tests (SPEC-ASS).  *Owner: composer.*
...
10. **History dedup is in-memory only; the on-disk file accumulates duplicates.** `_save_to_history()` in `_history.py` removes prior identical entries from the in-memory `self._history` list via `==` equality and **appends** the new line to `~/.hermes/.hermes_history` (no rewrite). Duplicates are collapsed only at next `_load_history()`. Comparison is content-exact, so trailing whitespace, case, or punctuation differences create distinct slots (e.g. `/help` and `/help ` both persist on disk).  *(Note: chord trigger via `Alt+$` REMOVED 2026-05-02 — use `$` prefix only. `SkillPickerOverlay` docstring updated to remove chord references.)* *Owner: composer (`_history.py`).*

11. ~~**`suggestion` field is an ad-hoc dynamic attribute.** Set with `# type: ignore[attr-defined]` at every write site; not declared on `HermesInput` or any base class. Works, but fragile under typing tools and inheritance changes.~~ **CLOSED 2026-05-02** — `suggestion` field declared at class level in `widget.py` (line 89: `suggestion: str = ""`). *Owner: composer.*
