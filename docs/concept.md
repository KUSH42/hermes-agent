...
**Note on streaming KIND hint (SLR-3).** A separate, glyph-only signal — `streaming_kind_hint` — may update the header icon and category chip during STREAMING based on a first-chunk sniff. It does not mutate `view_state.kind` and does not pick a renderer; the body keeps streaming through the raw renderer. Hint clears at COMPLETING; classifier output then drives both icon and renderer as today. The hint is a guess, the classifier is resolution. Header-side defensive clear in `tool_blocks/_header.py::_on_axis_change` on the `"state"` axis when transitioning into the inlined terminal/resolving set `{COMPLETING, DONE, ERROR, CANCELLED}` guards against late axis-write races (SK-2, 2026-04-27).
