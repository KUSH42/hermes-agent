...
- [Bundled TUI skins live under hermes_cli/skins](/home/xush/.hermes/feedback_bundled_tui_skins_design_md.md) — `build_skin_vars.scan_bundled_skins()` resolves bundled skins from `hermes_cli/skins/*/DESIGN.md`, not repo-root `skins/`; keep `x-hermes.component-vars` fully populated in bundled DESIGN.md frontmatter so drift tests do not rely on runtime fallback defaults
...
## ThinkingWidget Gradient + Animation Pools Spec (2026-05-02)
- [ThinkingWidget gradient + randomized animation pools spec](/home/xush/.hermes/2026-05-02-thinking-widget-gradient-randomization-spec.md) — IMPLEMENTED on a `feat/textual-migration` descendant worktree; running spinner shimmer now renders on `#spinner-overlay` with cached frame signatures, `_AnimSurface` paints per-cell braille gradients from `thinking-spinner-dim` / `thinking-spinner-peak`, and `tui.thinking.{engine,effect,long_wait_engine,long_wait_effect}` accepts scalar-or-list config values with one validated random pick per phase; targeted verification: `tests/tui/test_thinking_widget_v2.py` + `tests/tui/test_agent_running_ux.py` (62 tests)
