...
When you post results back to maintainers or in a PR, keep the wording careful.

Good examples:

- `Task-quality parity achieved on this eval run.`
- `Summary mode reduced model-visible payload on the current corpus.`
- `These results are still preliminary because the corpus is limited.`
...
If you are extending the evaluation effort rather than only running the
existing scripts, use the expansion plan in:

- `~/.hermes/2026-04-05-terminal-output-interceptor-eval-expansion-spec.md`

That spec defines the next fixture, task, repeat, and model-coverage targets.
