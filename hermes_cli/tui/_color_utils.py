...
def _hue_rotate(hex_color: str, delta: float) -> str:
    ...
    except Exception:  # il-ex-1-exempt: invalid hex input — return original string as-is
        return hex_color
...
def _rich_to_hex(value: str) -> str:
    ...
    except Exception:  # il-ex-1-exempt: rich colour parse failed — fall back to default cyan
        return "#00d7ff"
    ...
    if value.startswith("$"):
        ...
        try:
            ...
            if raw and raw.startswith("#") and len(raw) in (4, 7):
                ...
                resolved_hex = "#00d7ff"
        except Exception:  # il-ex-1-exempt: app.get_css_variables() unavailable (e.g. test env) — use default
            resolved_hex = "#00d7ff"
    else:
