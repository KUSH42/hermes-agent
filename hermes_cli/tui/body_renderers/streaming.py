    ...
    def finalize(self, all_plain: list[str], **kw: object) -> "ConsoleRenderable | None":
        ...
        if app is not None:
            try:
                ...
                if not swap:
                    return None
            except Exception:  # il-ex-1-exempt: config unavailable — proceed with swap (default=True behaviour)
                pass
        ...
        if text.startswith(("{", "[")):
            try:
                _json.loads(text)
                lang = "json"
            except Exception:  # il-ex-1-exempt: not valid JSON — lang stays None, no swap
                pass
        ...
        except Exception:  # il-ex-1-exempt: rich.Syntax init failed — fall back to plain text
            syntax = _Text(text)
...
class StreamingCodeRenderer(BodyRenderer):
    ...
    def finalize_code(
        ...
        with measure("renderer.finalize_code", budget_ms=20.0):
            ...
            except Exception:  # il-ex-1-exempt: rich.Syntax init failed — return plain text fallback
...
class StreamingSearchRenderer(BodyRenderer):
    ...
    def finalize(self, all_plain: list[str], **kwargs: object) -> "ConsoleRenderable | None":
        ...
        if joined.startswith("{"):
            ...
            except Exception:  # il-ex-1-exempt: JSON parse failed — fall back to plain-text listing
                pass
        result = Text()
    ...
    def extract_sidecar(self, tool_call: object, all_plain: list[str]) -> None:
        ...
        except Exception:  # il-ex-1-exempt: path extraction failed — sidecar data unavailable, non-critical
            pass
...
class MCPBodyRenderer(BodyRenderer):
    ...
    def finalize(self, all_plain: list[str], **kwargs: object) -> "ConsoleRenderable | None":
        ...
        try:
            ...
            if texts:
                return Text("\n\n".join(texts))
        except (json.JSONDecodeError, TypeError, KeyError):  # il-ex-1-exempt: noqa: bare-except
            pass
        return None
