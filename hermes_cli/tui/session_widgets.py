...
class NewSessionOverlay(Widget):
    """Modal overlay for creating a new parallel worktree session.

    Opened by Ctrl+W N or clicking + in SessionBar.
    """
    ...
    NewSessionOverlay.--visible { display: block; }
    NewSessionOverlay #ns-error { color: $error; height: 1; }
    NewSessionOverlay #ns-buttons { height: 3; }
    """
    ...
    def compose(self) -> ComposeResult:
        yield Static("[bold]New Session[/bold]", id="ns-title")
        yield Static("Branch name:", id="ns-branch-label")
        from textual.widgets import Input
        yield Input(placeholder="feat/my-feature", id="ns-branch-input")
        yield Static("Base:", id="ns-base-label")
        with Horizontal(id="ns-base-row"):
            yield Button("● current branch", id="ns-base-current", classes="--selected-base")
            yield Button("○ main", id="ns-base-main")
        yield Static("", id="ns-error")
        with Horizontal(id="ns-buttons"):
            yield Button("[ Create ]", id="ns-create", variant="primary")
    ...
    def show_overlay(self) -> None:
        ...
        try:
            from textual.widgets import Input
            self.query_one("#ns-branch-input", Input).focus()
        except Exception:
            pass
    ...
    def _do_create(self) -> None:
        ...
        if not branch:
            ...
            return
        try:
            self.app._create_new_session(branch, self._base, self)
        except Exception as exc:
            self._set_error(str(exc))


class MergeConfirmOverlay(Widget):
    """3-step merge flow: pre-check → strategy → confirm."""
    ...
    MergeConfirmOverlay.--visible { display: block; }
    MergeConfirmOverlay #merge-body { height: auto; max-height: 12; overflow-y: auto; }
    MergeConfirmOverlay #merge-strategy-row { height: 3; }
    MergeConfirmOverlay #merge-buttons { height: 3; }
    """
    ...
    def __init__(self, **kwargs: object) -> None:
        ...
        self._strategy: str = "squash"

    def compose(self) -> ComposeResult:
        yield Static("", id="merge-title")
        yield Static("", id="merge-body")
        with Horizontal(id="merge-strategy-row"):
            yield Button("Merge commit", id="mg-merge")
            yield Button("● Squash", id="mg-squash", classes="--selected-strategy")
            yield Button("Rebase", id="mg-rebase")
        with Horizontal(id="merge-buttons"):
            yield Button("[ Merge + close session ]", id="mg-confirm", variant="primary")
            yield Button("[ Merge only ]", id="mg-merge-only")
            yield Button("[ Cancel ]", id="mg-cancel")

    def show_for(self, session_id: str, diff_stat: str) -> None:
        self._session_id = session_id
        self._strategy = "squash"
        try:
            self.query_one("#merge-title", Static).update(
                f"[bold]Merge session:[/bold] {session_id}"
            )
            self.query_one("#merge-body", Static).update(diff_stat or "(no diff)")
        except Exception:
            pass
        self.add_class("--visible")
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if btn_id == "mg-cancel":
            ...
            event.stop()
            try:
                self.app._run_merge(self._session_id, self._strategy, close_on_success=True, overlay=self)
            except Exception:
                pass
        elif btn_id == "mg-merge-only":
            event.stop()
            try:
                self.app._run_merge(self._session_id, self._strategy, close_on_success=False, overlay=self)
            except Exception:
                pass
