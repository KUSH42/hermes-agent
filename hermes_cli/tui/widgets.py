...
# ---------------------------------------------------------------------------
# InlineProseLog — CopyableRichLog extension for mixed text + image lines
# ---------------------------------------------------------------------------
...
class CopyableBlock(Widget):
    ...
    def __init__(self, _log_id: str | None = None, _log_class: "type | None" = None, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        log_cls = _log_class if _log_class is not None else CopyableRichLog
        log_kwargs: dict[str, Any] = {"markup": False, "highlight": False, "wrap": True}
        if _log_id:
            log_kwargs["id"] = _log_id
        self._log = log_cls(**log_kwargs)
...
class MessagePanel(Widget):
    ...
    def _schedule_group_widget(self, new_panel: Widget) -> None:
        """Schedule async reparent of new_panel into a ToolGroup (v4 P5)."""
        self._group_reparent_worker(new_panel)

    @work(exclusive=False)
    async def _group_reparent_worker(self, new_panel: Widget) -> None:
        """Async reparent new_panel into a ToolGroup widget if a grouping rule fires."""
        try:
            ...
            match = _find_rule_match(self, new_panel)
            if match is None:
                return
            existing_panel, rule = match
            if _is_streaming(existing_panel):
                return
            # Guard: panels must still be mounted
            if not new_panel.is_attached or not existing_panel.is_attached:
                return
            existing_group = _get_tool_group(existing_panel)
            if existing_group is not None:
                await _do_append_to_group(existing_group, new_panel, self)
            else:
                await _do_apply_group_widget(self, existing_panel, new_panel, rule)
        except Exception:
            pass  # CSS grouping already applied by _maybe_start_group; safe to swallow
