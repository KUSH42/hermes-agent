...
class AIAgent:
        ...
        if self.api_mode == "bedrock_converse":
            ...
            def _bedrock_call():
                try:
                    ...
                    def _on_tool(name):
                        _fire_first()
                        self._fire_tool_gen_started(_bedrock_tool_idx[0], name)
                        _bedrock_tool_idx[0] += 1
    ...
    def _execute_tool_calls_concurrent(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        if self.tool_batch_callback:
            try:
                batch_payload = [(tc.id, name, args) for tc, name, args, _br, _bg in parsed_calls]
        ...
        try:
            # Filter out tool calls blocked by guardrails — those are handled
            # synchronously below without entering the executor.
            runnable_calls = [
                (i, tc, name, args)
                for i, (tc, name, args, block_result, blocked_by_guardrail) in enumerate(parsed_calls)
            ...
            if runnable_calls:
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                    for i, tc, name, args in runnable_calls:
                        ...
                        futures.append(f)
                        future_meta[f] = (i, tc, name, args)

                    # Drive completion via as_completed so SM-04 _emit_completion
                    # fires immediately as each tool finishes (TUI progressive
                    # update).  Interleave periodic heartbeats + interrupt checks
                    # by polling concurrent.futures.wait with a short timeout
                    # before the as_completed loop blocks indefinitely.
                    ...
                    while pending:
                        ...
                        for fut in done:
                            i_done, tc_done, name_done, args_done = future_meta[fut]
                            try:
                                fut.result()  # surface unhandled exceptions from _run_tool
                            except Exception:
                        ...
                        if self._interrupt_requested:
                            if not _interrupt_logged:
                                ...
                                self._vprint(
                                    ...
                                    f"{len(pending)} pending concurrent tool(s)",
                                    force=True,
                                )
                            for f in pending:
                                f.cancel()
                            ...
                            # Drain any that finished during the grace period.
                            for fut in pending:
                                if fut.done():
                                    i_done, tc_done, name_done, args_done = future_meta[fut]
                                    try:
                        ...
                        if _conc_elapsed > 0 and _conc_elapsed % 30 < 6:
                            _still_running = [
                                future_meta[f][2]  # name from (i, tc, name, args)
                                for f in pending
                            ]
                            self._touch_activity(
                                f"concurrent tools running ({_conc_elapsed}s, "
                                f"{len(pending)} remaining: {', '.join(_still_running[:3])})"
                            )
        finally:
            if spinner:
        ...
        for i, (tc, name, args, block_result, blocked_by_guardrail) in enumerate(parsed_calls):
            ...
            # Fire tool_complete_callback ONLY for guardrail-blocked tools — runnable
            # tools already fired via _emit_completion in the parallel executor loop.
            if blocked_by_guardrail and self.tool_complete_callback:
                try:
                    self.tool_complete_callback(tc.id, name, args, function_result)
                except Exception as cb_err:
                    logging.debug(f"Tool complete callback error: {cb_err}")
