...
Last revalidated: **2026-04-21. ~2990+ total TUI tests passing** (9 bake-dependent SDF morph tests skip cleanly via `@requires_pil_bake` — PIL/Python 3.13 FreeType incompatibility).

Recent changes (details → reference files):
- **Tool call UX audit pass 7 — 25 items, 62 tests** (2026-04-21): Branch `feat/tool-ux-audit-pass7-2026-04-21`; spec at `/home/xush/.hermes/tui-tool-ux-audit-pass7-spec.md`.
  **Phase A (streaming feedback):** A1 — footer action row hidden while block is streaming (not completed). A2 — `action_toggle_tail_follow` flashes "tail-follow N/A" when block already complete. A3 — auto-collapse flashes "▾ auto-collapsed (N lines)" for 0.8s. A4 — stall detection: `_last_line_time` used to show "⚠ stalled?" in microcopy when no output for 5s; `stalled` param added to `microcopy_line()`. A5 — copy flash includes byte size "(N kB)" when payload > 1KB.
  **Phase B (artifacts & footer):** B1 — truncated chip made bold with ⚠ prefix. B2 — `_ArtifactButton(TooltipMixin, Button)` subclass; artifact buttons show full path as tooltip. B3 — `_show_all_artifacts` reset to False on ToolPanel collapse. B4 — MCP remediation includes server name. B5 — HTTP 3xx status chips use `tone="warning"`.
  **Phase C (header & tail):** C1 — AGENT label always appends "…" on truncation. C2 — browse badge moved from `render()` into `_render_v4()` as first tail element. C3 — narrow-mode hint priority sort: error actions (r/x/e) first. C4 — first-focus one-shot "(Enter) toggle" flash via `_toggle_hint_shown` flag. C5 — covered by A1.
  **Phase D (navigation):** D1 — `_get_tool_group()` helper used in `_build_hint_text()` to show "⇧↵ peek" hint when panel is grouped. D2 — `/tools` filter placeholder shows prefix examples. D3 — secondary args row lazy-mounted on first non-empty call. D4 — collapsed error panels show "⚠ stderr (e)" dim suffix in header tail. D5 — `_streaming_tool_count` reactive on HermesApp; StatusBar shows "N tools" badge during multi-tool turns.
  **Phase E (error polish):** E1 — remediation inline as " · hint: {text}" on error chip (separate row removed). E2 — `action_open_primary()` wrapped in try/except with error flash. E3 — sort mode name shown in ToolsScreen header. E4 — `ToolGroup.recompute_aggregate()` adds `--group-has-error` CSS class when any child has error. E5 — `action_dismiss_all_error_banners()` on HermesApp; Shift+X dismisses all banners at once.
  Key gotchas: `_last_line_time` already existed on `StreamingToolBlock` (don't add `_last_append_time`); `_get_tool_group()` helper in `tool_group.py` handles ToolGroup ancestor detection; `_ArtifactButton(TooltipMixin, Button)` requires TooltipMixin import from `tooltip.py`; badge must be FIRST in `_render_v4()` tail (previously appended in `render()` after `_render_v4()` returned — wrong order).
  → `tool_blocks.py`, `tool_panel.py`, `tool_result_parse.py`, `tools_overlay.py`, `tool_group.py`, `streaming_microcopy.py`, `app.py`, `widgets.py`, `hermes.tcss`, `tests/tui/test_tool_ux_pass7_{a,b,c,d,e}.py` (62 new tests)
- **Tool call UX audit pass 5 — phases A–E, 28 items** (2026-04-21): 28 UX fixes across 5 phases; 87 new tests; branch `feat/tool-ux-audit-2026-04-21`; spec at `/home/xush/.hermes/tui-tool-ux-audit-pass5-spec.md`.
  **Phase A (header clarity):** A1 — sub-500ms fast-tool flash `"done"` when microcopy never shown (`_microcopy_shown`). A2 — `result-empty` CSS class + dim `"—"` hero chip on zero-line success. A3 — tail order reordered to hero→promoted→stat badges. A4 — `_secondary_args_snapshot` preserves secondary args across `complete()`. A5 — error banner `Static` mounted between header and body with `x` dismiss keybind. A6 — `is_first_in_turn` param + `first-in-turn` border on `ToolPanel` (counter reset in `watch_agent_running`; `self.parent.parent` to reach `ToolPanel` from `StreamingToolBlock`). A7 — `_tooltip_text` set to `_full_path` in `_render_v4` when truncated + `P` copy-full-path binding. A8 — MCP middle-truncation; `[MCP]` fallback when `avail < 8`.
  **Phase B (pagination):** B1 — OmissionBar positional label `"  N–M of Total  "`. B2 — page buttons show actual step `[↑+N]`/`[↓+N]` using module-level `_PAGE_SIZE`. B3 — `_cap_label` Static with `--visible` toggle. B4 — `WriteFileBlock.update_progress()` + `_progress_label` Static (NOT `render()` — ignored on compose() widget). B5 — `_MAX_HISTORY_LINES=10_000` / `_EVICT_CHUNK=500` eviction using `del _all_plain[:chunk]` (not deque — slicing required). B6 — `_saved_visible_start` in `ToolPanel.watch_collapsed`.
  **Phase C (error handling):** C1 — timeout/signal `error_kind` using `raw.lower()` (not `stderr` — no such local var). C2 — remediation hints for shell/code: timeout→timeout_sec, exit 127→PATH, SIGKILL→memory. C3 — `(partial)` suffix when `_visible_count < len(_all_plain)` via `self._panel._block`. C5 — `_flash_tone` attr on `ToolHeader`; `_flash_header(msg, tone=)` param; format-specific copy labels.
  **Phase D (navigation):** D1 — `_show_context_menu_at_center()` + `?` binding. D2 — `border-left: vkey $accent 60%` on `:focus`. D3 — state-aware hints (j/k only expanded, x only with banner, r only on error). D4 — `"↑ fewer"` collapse button + `--artifact-collapse` cleanup in rebuild loop. D5 — `ToolsScreen` category prefix filter (`file:` `shell:` `error:` etc.) + 3-mode sort (`s` key).
  **Phase E (polish):** E1 — `[>]`/`[✓]`/`[!]` text suffixes in `_accessible_mode`. E2 — `_truncated_line_count` incremented in `append_line` when byte cap hit. E3 — `bold=True` alongside `underline=True` in `_linkify_text`. E4 — `_format_duration_v4` extended to minutes/hours (param is ms). E5 — `ExecuteCodeBlock._code_line_count = len(_code_lines)` in `complete()`.
  Key gotchas: `self.parent` in `StreamingToolBlock` is `BodyPane` (not `ToolPanel`) — use `self.parent.parent`; `render()` silently ignored on compose()-based `WriteFileBlock` — use `Static` child; `raw.lower()` not `stderr.lower()` in `shell_result_v4`/`code_result_v4`; `_PAGE_SIZE` is module-level (no instance attr); D5 entries are `dict` — use `.get()`.
  → `hermes_cli/tui/tool_blocks.py`, `hermes_cli/tui/tool_panel.py`, `hermes_cli/tui/tool_result_parse.py`, `hermes_cli/tui/tools_overlay.py`, `hermes_cli/tui/write_file_block.py`, `hermes_cli/tui/execute_code_block.py`, `hermes_cli/tui/hermes.tcss`, `hermes_cli/tui/app.py`, `cli.py`, `tests/tui/test_tool_ux_pass5_{a,b,c,d,e}.py` (87 new tests)
- **Tool call UX audit pass 6** (2026-04-21): 10 issues (2 P0, 4 P1, 4 P2); spec at `/home/xush/.hermes/tui-ux-audit-pass6-spec.md`. 12 new tests.
  **P0 fixes:** `render_tool_row` dur_col: both `if d >= 1000` branches were identical (`f"({d}ms)"`) — fixed to `f"({d/1000:.1f}s)"` for ≥1000ms. `ExecuteCodeBlock.complete(is_error=True)` was unconditionally collapsing the inner header (`self._header.collapsed = True`) for ALL errors before ToolPanel forced the outer body open — visual mismatch (body visible, header shows `▸`). Fix: removed the `is_error` auto-collapse branch entirely; normal threshold-based collapse applies; ToolPanel owns error expansion.
  **P1 fixes:** `ToolsScreen.action_cursor_up/down` were calling `await self._rebuild()` on every arrow key — O(N) rebuild per keypress. Root cause: Textual's `_on_key` (binding dispatch) fires BEFORE `on_key` (user handler), so `event.prevent_default()` in `on_key` couldn't block the already-fired action. Fix: actions now use `lv.index = self._cursor` only; removed duplicate up/down handling from `on_key`. `ToolsScreen._update_pills()` called `btn.remove()` (deferred — posts `RemoveWidget` message) then immediately called `pills_row.mount()` — DuplicateId panic on 2nd+ calls because old buttons still in DOM. Fix: `call_after_refresh(lambda btns=btns: pills_row.mount(*btns))`. MCP streaming microcopy used `spec.name.split("__")[-1]` → method name (last segment); for `mcp__github__search_repos` showed "search_repos" not "github". Fix: `parts[1] if len(parts) >= 3 else parts[-1]`. `action_copy_html` was passing the temp file PATH to `_copy_text_with_hint` instead of the HTML content — users got `/tmp/hermes_copy_*.html` on clipboard. Fix: copy `html` content directly; write file as bonus with try/except.
  **P2 fixes:** `ToolGroup.on_click` was missing `event.stop()` — added `if hasattr(event, "stop"): event.stop()` for consistency with `ToolHeader`. `action_retry` returned silently when `rs is None or not rs.is_error` — now calls `self._flash_header("no error")` before returning. `ToolHeader.flash_complete()` deprecated wrapper removed: updated the one remaining caller in `StreamingToolBlock.complete()` at line 1649 to call `flash_success()` directly, then removed the method. `ExecuteCodeBlock._emit_code_line` and `finalize_code` truncated the string BEFORE syntax-highlighting (breaking multi-token spans at char 60); fix: highlight full line, then `full_rich[:60] if full_rich.cell_len > 60 else full_rich`.
  Key invariants: `Widget.remove()` is deferred (posts `RemoveWidget` message) — widgets remain in DOM until processed; mount after remove in same call causes DuplicateId; use `call_after_refresh` to defer mount. `ToolBodyContainer` display is CSS-class-controlled (`expanded` class → `display: block`; absent → `display: none` from DEFAULT_CSS); `ToolPanel.watch_collapsed` uses inline `styles.display` override which takes precedence over CSS class. Lines 562-564 in `set_result_summary_v4` that re-apply footer visibility after `update_summary_v4` are NOT redundant — Textual's reactive watcher only fires when `collapsed` changes value; if value unchanged (non-collapse path), `watch_collapsed` won't re-fire after summary populates footer.
  → `hermes_cli/tui/tools_overlay.py §render_tool_row/action_cursor_up/action_cursor_down/on_key/_update_pills`, `hermes_cli/tui/execute_code_block.py §complete/_emit_code_line/finalize_code`, `hermes_cli/tui/streaming_microcopy.py §MCP branch`, `hermes_cli/tui/tool_panel.py §action_copy_html/action_retry`, `hermes_cli/tui/tool_group.py §on_click`, `hermes_cli/tui/tool_blocks.py §flash_complete/StreamingToolBlock.complete`, `tests/tui/test_tools_overlay.py`, `tests/tui/test_execute_code_block.py`, `tests/tui/test_streaming_microcopy.py`, `tests/tui/test_tool_panel.py`, `tests/tui/test_tool_group.py`, `tests/tui/test_tool_blocks.py`
...
- **Drawille overlay v2 — signal enrichment, phase-aware carousel, burst/ceremony, ambient idle, positioning, presets** (2026-04-21): 6 phases, 45 new tests (102 total) in `tests/tui/test_anim_overlay.py` + fixes to `tests/tui/test_drawille_overlay.py`.
  **Phase A:** `DrawilleOverlayCfg` gains `error_hold_frames`, `phase_aware_carousel`, `phase_crossfade_speed`, `completion_burst_frames`, `ambient_enabled/heat/alpha/engine`, `position_margin`, `rail_width`, `rail_output_margin`. New signals: `"reasoning"` (heat 0.65), `"error"` (spike + revert after `error_hold_frames`), `"waiting"` (heat 0.2, suppresses fade-out). `app.py` wiring: `open_reasoning`→`signal("reasoning")`, `close_reasoning`→`signal("thinking")`, error tool close→`signal("error")`, `watch_approval_state`→`signal("waiting"/"thinking")` (**NOT** `watch_agent_running` — fires too late, approval already resolved).
  **Phase B:** Phase-aware carousel: `_PHASE_CATEGORIES` maps signal→engine category; `_current_phase` tracks last non-token signal; phase change triggers immediate crossfade; `_carousel_key` (separate from `_current_engine_key`) tracks carousel engine. Carousel skips advance during ambient mode.
  **Phase C:** Burst accumulation: `_burst_counter` increments on back-to-back tool signals, heat clamped to 1.5. Completion ceremony: `signal("complete")` sets `_completion_burst_frames=4` countdown; during countdown heat interpolation is gated; `_waiting` flag cleared on complete.
  **Phase D:** Ambient idle: tri-state `_visibility_state` ("hidden"/"active"/"ambient"); `_transition_to_ambient()` dims color via `_resolve_color(dim=ambient_alpha)`; `watch_agent_running(False)` triggers ambient when `ambient_enabled=True`. Ambient→active crossfade guaranteed by checking `_visibility_state != "ambient"` before advancing carousel index.
  **Phase E:** 9 position anchors (added top-center, bottom-center, mid-right, mid-left). Rail mode (rail-right/rail-left) uses `styles.offset` NOT `styles.dock` (dock is unreliable post-mount). `_has_nameplate()` queries app for nameplate to compute top_safe. `_apply_min_size_overlay` guarded with `try/except ScreenStackError` — teardown race when debounce timer fires after screen stack empty.
  **Phase F:** `_PRESETS` dict (6 built-ins: minimal/balanced/hacker/zen/sdf/immersive). `/anim preset <name>` subcommand. Merge-not-replace: `dataclasses.asdict(current_cfg)` + preset dict, then `DrawilleOverlayCfg(**merged)`. `commands.py` updated with preset subcommands.
  Key gotchas: `watch_approval_state` (not `watch_agent_running`) for "waiting" signal; rail mode uses `styles.offset` not `styles.dock`; `_carousel_key` is separate from `_current_engine_key`; `WaveInterferenceEngine` (v2) heavier than original wave — excluded from 8ms timing budget test.
  → `hermes_cli/tui/drawille_overlay.py §_PHASE_CATEGORIES/_PRESETS/DrawilleOverlayCfg/DrawilleOverlay._apply_layout/_get_carousel_engine/_transition_to_ambient/signal`, `hermes_cli/tui/app.py §_apply_min_size_overlay/watch_approval_state/open_reasoning/close_reasoning`, `hermes_cli/commands.py §anim subcommands`, `tests/tui/test_anim_overlay.py` (+45 tests)
