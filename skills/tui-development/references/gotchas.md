...
- `ThinkingWidget` and its mounted `_AnimSurface` / `_LabelLine` children must each paint the resolved app background, but for these widgets that should happen via runtime `styles.background` assignment, not `$app-bg` in `DEFAULT_CSS`. The isolated `test_thinking_widget_v2.py` app does not load Hermes's custom CSS variable map, so widget-local `$app-bg` references raise `UnresolvedVariableError` before mount. Keep the cached `_app_bg_hex` / `_background_hex` values aligned with the render path so Rich `Segment` backgrounds and mounted widget backgrounds stay in sync.
