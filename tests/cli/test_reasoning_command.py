...
class TestCloseReasoningBoxDrainsMachines(unittest.TestCase):
    ...
    @patch("cli._cprint")
    def test_drain_output_before_closing_border(self, mock_cprint):
        """Drained content must appear in cprint output before the └ border."""
        cli = _make_close_reasoning_cli()
        cli._reasoning_block_buf.flush.return_value = None
        ...
        drain_idx  = next(i for i, c in enumerate(calls) if "drained" in c)
        border_idx = next(i for i, c in enumerate(calls) if "└" in c)
        self.assertLess(drain_idx, border_idx)
