...
from agent.display import (
    ...
    get_tool_display_tier,
    set_tool_tier_overrides,
...
# ---------------------------------------------------------------------------
# Tool tier classification — get_tool_display_tier / set_tool_tier_overrides
# ---------------------------------------------------------------------------

class TestToolDisplayTier:
    ...
    def test_verbose_tools(self):
        for tool in ("write_file", "patch", "terminal", "execute_code"):
            assert get_tool_display_tier(tool) == "verbose", f"{tool} should be verbose"

    def test_compact_tools(self):
        for tool in ("read_file", "search_files", "web_search", "web_extract"):
            assert get_tool_display_tier(tool) == "compact", f"{tool} should be compact"

    def test_silent_tools(self):
        for tool in ("todo", "memory", "session_search", "clarify"):
            assert get_tool_display_tier(tool) == "silent", f"{tool} should be silent"

    def test_unknown_tool_defaults_to_compact(self):
        assert get_tool_display_tier("my_custom_mcp_tool") == "compact"
        assert get_tool_display_tier("") == "compact"

    def test_override_applies_and_takes_precedence(self):
        set_tool_tier_overrides({"read_file": "verbose", "terminal": "silent"})
        assert get_tool_display_tier("read_file") == "verbose"
        assert get_tool_display_tier("terminal") == "silent"
        # Unaffected tools still use defaults
        assert get_tool_display_tier("write_file") == "verbose"
        assert get_tool_display_tier("todo") == "silent"

    def test_invalid_tier_in_override_is_ignored(self):
        set_tool_tier_overrides({"read_file": "loud", "terminal": "verbose"})
        # "loud" is not a valid tier — read_file should fall back to its default
        assert get_tool_display_tier("read_file") == "compact"
        # "verbose" is valid — terminal override applies
        assert get_tool_display_tier("terminal") == "verbose"


# ---------------------------------------------------------------------------
# Prefix kwarg on preview renderers
# ---------------------------------------------------------------------------

class TestPreviewRendererPrefix:
    """All four preview renderers must prepend *prefix* to every content line."""
    ...
    def test_render_edit_diff_prefix_on_content_lines(self):
        calls = []
        render_edit_diff_with_delta(
            "patch",
            '{"diff": "--- a/x\\n+++ b/x\\n@@ -1 +1 @@\\n-old\\n+new\\n"}',
        ...
        # The first call is the "review diff" header — it uses fixed "  ┊ " internally
        # Content lines (the actual diff lines) must start with the prefix
        content_lines = [self._strip_ansi(c) for c in calls[1:]]
        assert content_lines, "expected at least one content line"
        for line in content_lines:
            assert line.startswith(self._PREFIX), f"content line missing prefix: {line!r}"

    def test_render_read_file_prefix_on_content_lines(self):
        calls = []
        render_read_file_preview(
            "app.py",
            '{"content": "x = 1\\ny = 2"}',
        ...
        # First call is the header line; content lines follow
        content_lines = [self._strip_ansi(c) for c in calls[1:]]
        assert content_lines, "expected at least one content line"
        for line in content_lines:
            assert line.startswith(self._PREFIX), f"content line missing prefix: {line!r}"

    def test_render_terminal_prefix_on_content_lines(self):
        calls = []
        render_terminal_preview(
            "cat app.py",
            '{"output": "x = 1\\ny = 2"}',
    ...
    def test_render_execute_code_prefix_on_content_lines(self):
        calls = []
        render_execute_code_preview(
            "x = 1\ny = 2",
            print_fn=calls.append,
        ...
        # execute_code preview has no header line — all calls are content
        assert calls, "expected at least one content line"
        for line in calls:
            stripped = self._strip_ansi(line)
            # content lines are indented: prefix + "  " + code
            assert stripped.startswith(self._PREFIX), f"content line missing prefix: {stripped!r}"

    def test_empty_prefix_preserves_existing_indentation(self):
        """prefix='' should produce same output as no prefix arg."""
        calls_no_prefix = []
        calls_empty = []
        render_execute_code_preview("x = 1", print_fn=calls_no_prefix.append)
        render_execute_code_preview("x = 1", print_fn=calls_empty.append, prefix="")
        assert [re.sub(r"\x1b\[[0-9;]*m", "", c) for c in calls_no_prefix] == \
               [re.sub(r"\x1b\[[0-9;]*m", "", c) for c in calls_empty]


# ---------------------------------------------------------------------------
# Duration alignment — get_cute_tool_message width param + _pad_dur
# ---------------------------------------------------------------------------

class TestCuteMessageDurationAlignment:
    def _strip_ansi(self, s: str) -> str:
        return re.sub(r"\x1b\[[0-9;]*m", "", s)

    def test_duration_always_present(self):
        msg = get_cute_tool_message("read_file", {"path": "/tmp/f.py"}, 0.5)
        assert "0.5s" in msg

    def test_duration_right_aligned_at_width(self):
        """With an explicit width, the duration should appear near the right edge."""
        width = 80
        msg = self._strip_ansi(
            get_cute_tool_message("read_file", {"path": "/tmp/f.py"}, 1.2, width=width)
        )
        assert "1.2s" in msg
        dur_pos = msg.rfind("1.2s")
        # duration + failure suffix should be within the last 8 chars of the target
        assert dur_pos >= width - 20, f"duration too far left at pos {dur_pos} in {msg!r}"

    def test_narrow_width_falls_back_gracefully(self):
        """width=30 is narrower than any tool body — no crash, duration still present."""
        msg = get_cute_tool_message("read_file", {"path": "/tmp/f.py"}, 0.3, width=30)
        assert "0.3s" in msg

    def test_different_widths_produce_different_padding(self):
        """A wider terminal produces more padding before the duration."""
        msg_narrow = self._strip_ansi(
            get_cute_tool_message("read_file", {"path": "/a.py"}, 1.0, width=50)
        )
        msg_wide = self._strip_ansi(
            get_cute_tool_message("read_file", {"path": "/a.py"}, 1.0, width=100)
        )
        # The wide message should have more spaces before the duration
        spaces_narrow = msg_narrow.count(" ")
        spaces_wide = msg_wide.count(" ")
        assert spaces_wide > spaces_narrow
