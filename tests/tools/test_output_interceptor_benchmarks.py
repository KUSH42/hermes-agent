...
from scripts.output_interceptor_bench import run_benchmark, write_chart_bundle, write_detailed_report
...
def test_output_interceptor_benchmark_acceptance_targets():
    ...
    assert aggregate["median_reduction_pct"] >= 45.0
    assert aggregate["median_delta_ms"] <= 10.0
    assert aggregate["raw_recovery_rate_pct"] == 100.0
    assert aggregate["serialization_mode"] == "production"

    assert rows["git_diff_small"]["reduction_pct"] > 0.0
    assert rows["git_diff_medium"]["reduction_pct"] >= 50.0
    assert rows["git_diff_large"]["reduction_pct"] >= 75.0
    assert rows["pytest_noisy"]["reduction_pct"] >= 75.0
...
def test_output_interceptor_benchmark_writes_chart_bundle(tmp_path):
    ...
    report_path = write_detailed_report(results, tmp_path, charts)
