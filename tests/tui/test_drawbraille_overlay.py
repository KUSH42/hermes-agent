"""Tests for DrawbrailleOverlay, AnimConfigPanel, and animation engines.
...
def _default_cfg(**overrides: object) -> DrawbrailleOverlayCfg:
...
def test_hidden_when_disabled():
    ...
    ov = DrawbrailleOverlay()
    cfg = DrawbrailleOverlayCfg(enabled=False)
    ov._start_anim = MagicMock()
    ov.show(cfg)
...
def test_anim_gallery_is_modal_screen():
    ...
    from textual.widget import Widget
    from hermes_cli.tui.drawbraille_overlay import AnimGalleryOverlay
...
def test_drawbraille_overlay_cfg_multi_color_defaults():
    """DrawbrailleOverlayCfg defaults: multi_color=[], hue_shift_speed=0.3."""
...
def test_drawbraille_overlay_default_css_no_hardcoded_offset():
    """DEFAULT_CSS must not hardcode 'offset: 15 5' — position set by _apply_layout."""
    assert "offset: 15 5" not in DrawbrailleOverlay.DEFAULT_CSS


def test_hermes_tcss_has_layer_overlay_for_drawbraille():
    """hermes.tcss must declare layer:overlay and position:absolute for DrawbrailleOverlay.
...
def test_anim_gallery_overlay_not_in_default_css():
    ...
    (absolute positioning is reserved for DrawbrailleOverlay which needs manual offset).
    """
    from hermes_cli.tui.drawbraille_overlay import AnimGalleryOverlay
