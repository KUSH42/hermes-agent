"""Tests for drawille animation performance optimisations.
...
def test_render_multi_color_divisor_hoisted():
    ...
    frame = "abc\ndef"
    result = DrawilleOverlay._render_multi_color(stub, frame, 0.0)  # type: ignore[arg-type]
...
def _make_multi_color_stub(colors, hue_shift_speed=0.3):
    """Build a minimal stub for DrawilleOverlay._render_multi_color."""
    from hermes_cli.tui.animation import _parse_rgb
...
def test_render_multi_color_buffer_reuse():
    ...
    frame = "abcde\nfghij"
    DrawilleOverlay._render_multi_color(stub, frame, 0.0)  # type: ignore[arg-type]
    buf_id = id(stub._multi_color_row_buf)

    DrawilleOverlay._render_multi_color(stub, frame, 0.1)  # type: ignore[arg-type]
