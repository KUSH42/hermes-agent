...
async def test_click_left_toggles_expanded():
    """Left-click on ToolBlock header collapses the ToolPanel."""
    from hermes_cli.tui.tool_panel import ToolPanel
    app = _make_app()
    async with app.run_test(size=(80, 24)) as pilot:
        lines = [f"L{i}" for i in range(COLLAPSE_THRESHOLD + 1)]
        ...
        assert panel.collapsed is True  # click toggles via action_toggle_collapse
...
async def test_click_left_toggles_back_to_collapsed():
    ...
    async with app.run_test(size=(80, 24)) as pilot:
        ...
        block = app.query_one(ToolBlock)
        panel = app.query_one(ToolPanel)
        initial_collapsed = panel.collapsed  # starts expanded
...
async def test_click_calls_toggle_exactly_once():
    """Left-click on ToolBlock header calls action_toggle_collapse exactly once."""
    from unittest.mock import patch as _patch
    from hermes_cli.tui.tool_panel import ToolPanel
    app = _make_app()
    async with app.run_test(size=(80, 24)) as pilot:
