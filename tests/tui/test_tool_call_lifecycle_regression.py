...
class TestRendererExceptionLogging:
    ...
    def test_renderer_swap_exception_logs_debug_exc_info(self, caplog):
        """A forced renderer swap failure logs with renderer class and exc_info."""
        panel = self._make_panel_mixin()

        mock_renderer_cls = MagicMock()
        mock_renderer_cls.__name__ = "FakeRenderer"
        mock_renderer_cls.side_effect = RuntimeError("swap failed")
        ...
        with caplog.at_level(logging.DEBUG, logger="hermes_cli.tui.tool_panel._completion"):
            panel._swap_renderer(mock_renderer_cls, mock_payload, mock_result)
    ...
    def test_renderer_swap_failure_keeps_old_block(self):
        ...
        original_block.is_attached = True
        panel._block = original_block

        mock_body_pane = MagicMock()
        mock_body_pane.mount = MagicMock(side_effect=RuntimeError("mount failed"))
        panel._body_pane = mock_body_pane
        ...
        panel._swap_renderer(mock_renderer_cls, MagicMock(), mock_result)
