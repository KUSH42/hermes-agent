...
class TestKindRevertBinding:
    ...
    def test_revert_hint_in_contextual_when_override_active(self):
        """When user_kind_override is set, hint text should show 'T auto'."""
        from hermes_cli.tui.tool_panel._actions import _ToolPanelActionsMixin
        from hermes_cli.tui.tool_payload import ResultKind

        rs_mock = MagicMock()
        rs_mock.is_error = False
        rs_mock.stderr_tail = None
        rs_mock.actions = []
        rs_mock.artifacts = []
        ...
        panel = FakePanel()
        text = _ToolPanelActionsMixin._build_hint_text(panel)
        plain = text.plain
        # JSON override → contextual[0]=("t","as diff"), contextual[1]=("T","auto")
        # _build_hint_text shows contextual[:2] so both appear.
        assert "T" in plain and "auto" in plain, f"Expected T/auto in hint, got: {plain!r}"
...
class TestNextKindHint:
    ...
    def test_hint_label_uses_next_kind(self):
        ...
        text = _ToolPanelActionsMixin._build_hint_text(panel)
        plain = text.plain
        # JSON override → next kind is DIFF → hint text "as diff"
        assert "diff" in plain, f"Expected 'diff' in hint text, got: {plain!r}"
